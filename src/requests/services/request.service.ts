...
export class RequestService {
  ...
  async assign(
    ...
    // If userId is null or undefined, it means we're unassigning
    let targetUser = null;
    if (assignRequestDto.userId !== null && assignRequestDto.userId !== undefined) {
    ...
    // Update assignee
    // TypeScript needs explicit type handling here
    if (assignRequestDto.userId !== null && assignRequestDto.userId !== undefined) {
      request.assignedToId = assignRequestDto.userId;
    } else {
    ...
    if (
      assignRequestDto.userId !== null &&
      assignRequestDto.userId !== undefined &&
    ...
    if (
      (assignRequestDto.userId === null || assignRequestDto.userId === undefined) &&
      request.status?.name === 'В работе'
    ) {
      const newStatus = await this.statusRepository.findOne({
        where: { name: 'Новая' },
      });
    ...
    await this.logActivity(
      ...
      assignRequestDto.comment ||
        `Request ${assignRequestDto.userId !== null && assignRequestDto.userId !== undefined ? 'assigned to' : 'unassigned from'} ${newAssigneeName}`,
      oldAssigneeName,
      newAssigneeName,
    );
  ...
  private async findOneEntity(id: string, relations: string[] = []): Promise<Request> {
    const queryOptions: FindOneOptions<Request> = {
      where: { id },
    };
  ...
  private async logActivity(
    ...
    // Create a fully populated activity object to satisfy TypeScript
    const activity: Partial<RequestActivity> = {
      requestId,
      userId,
      type,
      message,
      ...
      user: undefined, // Will be populated by relation
      request: undefined, // Will be populated by relation
    };

    return this.activityRepository.save(activity as RequestActivity);
  }
